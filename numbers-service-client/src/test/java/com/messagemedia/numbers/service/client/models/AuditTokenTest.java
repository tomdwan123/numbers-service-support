...
public class AuditTokenTest {

    @Test
    public void testAccessors() throws Exception {
        assertGetters(randomAuditToken());
    }

    @Test
    public void testToString() {
        assertToString(randomAuditToken());
    }

    @Test
    public void shouldSerializeAndDeserializeCorrectly() throws Exception {
        JsonFastMapper mapper = new JsonConfig().fastMapper();
        String s = mapper.toJsonString(randomAuditToken());
        AuditToken fromJson = mapper.readFrom(s, AuditToken.class);
